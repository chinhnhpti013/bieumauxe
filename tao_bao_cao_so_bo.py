...
for row in ws_pt.iter_rows(min_row=2, values_only=True):
    if row[0] and 'Phương án hợp lệ' not in str(row[0]):
        phu_tung_all.append(str(row[0]))
for i in range(1, 15):
    info[f'ten_phu_tung_{i}'] = phu_tung_all[i-1] if i-1 < len(phu_tung_all) else ''

info['Dien_bien_tai_nan'] = info.get('dien_bien_tai_nan', '')

# ── Sinh file ─────────────────────────────────────────────────────────
make_docx(
    os.path.join(BASE_DIR, 'assets', 'Báo-cáo-sơ-bộ-giám-định.docx'),
    os.path.join(BASE_DIR, 'output', 'Bao-cao-so-bo.docx'),
    info
)
